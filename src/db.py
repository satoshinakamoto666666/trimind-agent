...
def record_decision(conn: sqlite3.Connection, decision: dict) -> int:
    cur = conn.execute(
        "INSERT INTO decisions (timestamp, consensus, action, votes_json, reasoning, executed, result) VALUES (?,?,?,?,?,?,?)",
        (
            int(time.time()),
            int(decision.get("consensus", False)),
            decision.get("action", "none"),
            json.dumps(decision.get("votes", {}), default=str),
            decision.get("reasoning", ""),
            int(decision.get("executed", False)),
            decision.get("result", ""),
        ),
    )
    conn.commit()
...
def update_decision_result(conn: sqlite3.Connection, decision_id: int, executed: bool, result: str = ""):
    conn.execute(
        "UPDATE decisions SET executed=?, result=? WHERE id=?",
        (int(executed), result[:4000], decision_id),
    )
    conn.commit()
